...
p = ggplot(subset(dat,year=="1970" | year=="2014"),
  ...
  ggtitle("Financial Sector Employee Compensation as Share of GDP, 1970 and 2014") +
  theme(panel.background=element_blank(),
        axis.text.x=element_text(family="Verdana",size=12,angle=0),
        axis.text.y=element_text(family="Verdana",size=12),
        axis.line.y=element_line(size=.15),
        axis.line.x=element_line(size=.15),
        axis.title.x=element_text(family="Verdana",size=14),
        axis.title.y=element_text(family="Verdana",size=14),
...
p2 = ggplot(dat,aes(x=year,y=perc.gdp, fill=perc.gdp)) +
  geom_bar(stat="identity",width=.65) +
  labs(x="Year",y="Percent of GDP") +
  scale_y_continuous(breaks = seq(0,max(dat$perc.gdp),by=.5),
                     expand = c(0,0),
  ...
  theme(panel.background=element_blank(),
        axis.text.x=element_text(family="Verdana",size=12,angle=90,vjust=.5),
        axis.text.y=element_text(family="Verdana",size=12),
        axis.line.y=element_line(size=.15),
        axis.line.x=element_line(size=.15),
        axis.title.x=element_text(family="Verdana",size=14),
